<?php include("frames/_head.php") ?>
<div class="boxed-container">
	<div class="main-container">
		<main>
			<!-- Page Banner -->
			<div class="page-banner container-fluid no-left-padding no-right-padding">
				<!-- Container -->
				<div class="container">
					<div class="page-banner-content">
						<h3>PATOLOGÍAS</h3>
					</div>
					<div class="banner-content">
						<ol class="breadcrumb">
							<li><a href="index.php">Inicio</a></li>
							<li class="active">Patologías</li>
						</ol>
					</div>
				</div><!-- Container /- -->
			</div><!-- Page Banner -->

			<?php include("modules/_patologias.php") ?>

			<!-- Offer Section -->
			<div class="offer-section container-fluid no-left-padding no-right-padding">
				<!-- Container -->
				<div class="container">
					<!-- Section Header -->
					<div class="section-header">
						<h3>What We Offer</h3>
					</div><!-- Section Header /- -->
					<div class="row">
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="offer-box">
								<i class="fa fa-hospital-o"></i>
								<h5>Intensive Care</h5>
								<p>Mister we could use a man like Herbert Hoover again. Love exciting and new. Come aboardwere expecting you.</p>
								<a href="#" title="Read More">Read More</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="offer-box">
								<i class="fa fa-ambulance"></i>
								<h5>24/7 Ambulance</h5>
								<p>In crack commando was sent to prison by a military court for a crime they didn't commit. These men prompt-</p>
								<a href="#" title="Read More">Read More</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="offer-box">
								<i class="fa fa-smile-o"></i>
								<h5>Friendly Doctors</h5>
								<p>Come aboard expecting you. Love life's sweetest reward Let it flow it floats back to you. It's a beautiful Day.</p>
								<a href="#" title="Read More">Read More</a>
							</div>
						</div>
					</div>
				</div><!-- Container -->
			</div><!-- Offer Section /- -->

			<!-- Other Services -->
			<div class="other-services container-fluid no-left-padding no-right-padding">
				<!-- Container -->
				<div class="container">
					<!-- Section Header -->
					<div class="section-header">
						<h3>Other Services</h3>
					</div><!-- Section Header /- -->
					<div class="row srv-box">
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-heart"></i>
								</div>
								<div class="other-services-content">
									<h5>Acute Rehabilitation</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-thumbs-up"></i>
								</div>
								<div class="other-services-content">
									<h5>Adaptive Sports</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-users"></i>
								</div>
								<div class="other-services-content">
									<h5>Adolescent Medicine</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-phone-square"></i>
								</div>
								<div class="other-services-content">
									<h5>Assistive Technology</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-graduation-cap"></i>
								</div>
								<div class="other-services-content">
									<h5>Back to School</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="other-services-block">
								<div class="services-block-icon">
									<i class="fa fa-male"></i>
								</div>
								<div class="other-services-content">
									<h5>Brain Injury program</h5>
									<p>Dolor sit amet consecdi pisicing eliamsed do eiusmod tempornu</p>
								</div>
							</div>
						</div>
					</div>
				</div><!-- Container /- -->
			</div><!-- Other Services -->

			<!-- Extra Services -->
			<div class="extra-services container-fluid no-left-padding no-right-padding">
				<!-- Container -->
				<div class="container">
					<!-- Section Header -->
					<div class="section-header">
						<h3>Extra Services</h3>
					</div><!-- Section Header /- -->
					<div class="row">
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-hospital-o"></i>
								<h5>web Design</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-wheelchair-alt"></i>
								<h5>Mobility Equipments</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-medkit"></i>
								<h5>First Aid kits</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-heartbeat"></i>
								<h5>Heart Check</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-stethoscope"></i>
								<h5>AMSR Checkup</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 col-xs-6">
							<div class="extra-box">
								<i class="fa fa-user-md"></i>
								<h5>Private Care</h5>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris morbi accumsan.</p>
								<a href="#" title="View details">View details</a>
							</div>
						</div>
					</div>
				</div><!-- Container /- -->
			</div><!-- Extra Services /- -->

			<!-- Tab Section -->
			<div class="tab-section container-fluid no-left-padding no-right-padding">
				<!-- Container -->
				<div class="container">
					<div class="tab-detail">
						<!-- Nav tabs -->
						<ul class="nav nav-tabs services-tabs" role="tablist">
							<li role="presentation" class="active"><a href="#tab-neurology" role="tab" data-toggle="tab">Neurology</a></li>
							<li role="presentation"><a href="#tab-hematology" class="tab-review" role="tab" data-toggle="tab">Hematology</a></li>
							<li role="presentation"><a href="#tab-dermatology" class="tab-review" role="tab" data-toggle="tab">Dermatology</a></li>
							<li role="presentation"><a href="#tab-pediatrician" class="tab-review" role="tab" data-toggle="tab">Pediatrician</a></li>
							<li role="presentation"><a href="#tab-radiation" class="tab-review" role="tab" data-toggle="tab">Radiation</a></li>
						</ul>
						<!-- Tab panes -->
						<div class="tab-content">
							<div role="tabpanel" class="tab-pane active" id="tab-neurology">
								<i><img src="assets/images/tab-1.png" alt="tab" /></i>
								<h5>Neurology Services</h5>
								<p>This is Photoshop's version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut im</p>
							</div>
							<div role="tabpanel" class="tab-pane" id="tab-hematology">
								<i><img src="assets/images/tab-1.png" alt="tab" /></i>
								<h5>hematology Services</h5>
								<p>This is Photoshop's version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut im</p>
							</div>
							<div role="tabpanel" class="tab-pane" id="tab-dermatology">
								<i><img src="assets/images/tab-1.png" alt="tab" /></i>
								<h5>dermatology Services</h5>
								<p>This is Photoshop's version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut im</p>
							</div>
							<div role="tabpanel" class="tab-pane" id="tab-pediatrician">
								<i><img src="assets/images/tab-1.png" alt="tab" /></i>
								<h5>pediatrician Services</h5>
								<p>This is Photoshop's version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut im</p>
							</div>
							<div role="tabpanel" class="tab-pane" id="tab-radiation">
								<i><img src="assets/images/tab-1.png" alt="tab" /></i>
								<h5>radiation Services</h5>
								<p>This is Photoshop's version of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit.</p>
								<p>Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut im</p>
							</div>
						</div><!-- Tab panes /- -->
					</div>
				</div><!-- Container /- -->
			</div><!-- Tab Section -->
		</main>
	</div>
	<?php include("frames/_foot.php") ?>